...
namespace Origination.Service
{
    public class ApplicationService : IApplicationService
    {
        ...
        public Application GetApplication(Guid applicationId)
        {
            ...
            activity?.SetTag("ApplicationId", applicationId);
        ...
        public void CreateApplication(Application application)
        {
            ...
            activity?.SetTag("ApplicationId", application.ApplicationId);
        ...
        public void UpdateApplication(Application application)
        {
            ...
            {
                // This will be thrown by GetApplication if the application doesn't exist
                activity?.SetStatus(ActivityStatusCode.Error, ex.Message);
                throw;
            }
            catch (Exception ex)
        ...
        public void SubmitApplicationFile(Guid applicationId, DocumentType docuType, string fileName, Stream file)
        {
            ...
            activity?.SetTag("ApplicationId", applicationId);
            activity?.SetTag("DocumentType", docuType.ToString());
            ...
            try
            {
                ...
                activity?.SetTag("S3Key", s3Key);
                activity?.SetTag("BucketName", bucketName);
                ...
                {
                    uploadActivity?.SetTag("S3Key", s3Key);
                    uploadActivity?.SetTag("BucketName", bucketName);
        ...
        {
            using var activity = _tracer.StartActivity("PublishEvent", ActivityKind.Producer, parentActivity?.Context ?? default);
            activity?.SetTag("EventName", eventName);
            
            try
            {
                // Create a dictionary to hold both the payload and trace context
                var eventPayload = new Dictionary<string, object>
                {
                    { "payload", payload },
                    // Add trace context for downstream services to continue the trace
                    { "traceContext", new {
                        TraceId = activity?.Context.TraceId.ToString() ?? parentActivity?.Context.TraceId.ToString(),
                        SpanId = activity?.Context.SpanId.ToString() ?? parentActivity?.Context.SpanId.ToString(),
                        TraceFlags = activity?.Context.TraceFlags.ToString() ?? parentActivity?.Context.TraceFlags.ToString()
                    }}
                };
                    ...
                    Entries = new List<Amazon.EventBridge.Model.PutEventsRequestEntry>()
                                {
                                    new Amazon.EventBridge.Model.PutEventsRequestEntry()
                                    {
                                        ...
                                        Detail = JsonSerializer.Serialize(eventPayload),
